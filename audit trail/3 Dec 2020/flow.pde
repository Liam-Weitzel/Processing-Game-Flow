...
void draw() {
  ...
  /* drawing map temp (change into function)*/
  float[] level1 = {0, height-30, width, 30, 0, height-300, width/2, 100, width-200, height/3, 200, 20, 50, height/5, 100, 20};
  fill(0,70,100);
  rect(level1[0],level1[1],level1[2],level1[3]);
  rect(level1[4],level1[5],level1[6],level1[7]);
  rect(level1[8],level1[9],level1[10],level1[11]); 
  rect(level1[12], level1[13], level1[14], level1[15]);
  ...
  if ((grounded(level1) == true && (nearestfloor(level1)-playerpos.y <= 20 || nearestroof(level1)-playerpos.y <= 20)) || playerpos.y > height-(playersize/2)) {
    ...
    if (((playerfloor*playerfloor) < (playerroof*playerroof)) && ((playersideleft*playersideleft) > (playerfloor*playerfloor)) && ((playersideright*playersideright) > (playerfloor*playerfloor))) {
      ...
      print("right");
      println();
    } else if (((playerfloor*playerfloor) > (playersideleft*playersideleft)) && ((playersideleft*playersideleft) < (playersideright*playersideright)) && ((playerroof*playerroof) > (playersideleft*playersideleft))) {
...
float nearestfloor(float[] arr) {
  ...
  for(int i = rectx.length-1; i >= 0; i--) {
    if (playerpos.y-5 <= recty[i]){
      if ((playerpos.x >= rectx[i]) && (playerpos.x <= (rectx[i]+rectw[i]))) {
        return recty[i];
      }
    }
  }
